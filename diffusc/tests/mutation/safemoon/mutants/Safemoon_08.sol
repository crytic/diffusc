...
contract Safemoon_08 is ISafemoon, Initializable, ContextUpgradeable, OwnableUpgradeable {
    ...
    function _getValues(uint256 tAmount, uint256 _tierIndex) private view returns (FeeValues memory) {
        ...
        (uint256 rAmount, uint256 rTransferAmount, uint256 rFee) = _getRValues(
            uint128(tAmount),
            uint128(tValues.tFee),
            uint128(tTransferFee),
            uint128(_getRate())
        );
        return
            FeeValues(
    ...
    {
        uint128 rAmount = uint128(tAmount.mul(currentRate));
        uint128 rFee = uint128(tFee.mul(currentRate));
        uint128 rTransferFee = uint128(tTransferFee.mul(currentRate));
        uint128 rTransferAmount = uint128(rAmount.sub(rFee).sub(rTransferFee));
