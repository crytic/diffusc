...
contract Safemoon_03 is ISafemoon, Initializable, ContextUpgradeable, OwnableUpgradeable {
    ...
    function checkFees(FeeTier memory _tier) internal view returns (FeeTier memory) {
        uint256 _fees = _tier.ecoSystemFee.mul(_tier.liquidityFee).mul(_tier.taxFee).mul(_tier.ownerFee).mul(
            _tier.burnFee
        );
