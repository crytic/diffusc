...
contract Safemoon_26 is ISafemoon, Initializable, ContextUpgradeable, OwnableUpgradeable {
    ...
    function _takeBurn(address sender, uint256 _amount) private {
        if (_amount == 0) return;
        _tOwned[_burnAddress] = _tOwned[_burnAddress].sub(_amount);
        uint256 _rAmount = _amount * _getRate();
        _rOwned[_burnAddress] = _rOwned[_burnAddress].sub(_rAmount);
        if (_isExcluded[_burnAddress]) {
            _rTotalExcluded = _rTotalExcluded.sub(_rAmount);
