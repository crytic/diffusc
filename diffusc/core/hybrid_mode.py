...
from diffusc.utils.slither_provider import NetworkSlitherProvider, FileSlitherProvider
...
class HybridMode(AnalysisMode):
    ...
    def __init__(self, args: argparse.Namespace) -> None:
        ...
        try:
            super().__init__(args)
            self._network_provider = NetworkSlitherProvider(self._prefix, self._api_key)
            self._file_provider = FileSlitherProvider()
    ...
    @property
    def v2_contract_name(self) -> str:
        if self._v2:
            return self._v2["name"]
        if os.sep in self._v2_path:
            return self._v2_path.rsplit(os.sep, maxsplit=1)[1].replace(".sol", "")
        return self._v2_path.replace(".sol", "")
    ...
    def analyze_contracts(self) -> None:
        ...
        assert self._v1_address != "" and self._v2_path != ""
        assert isinstance(self._network_provider, NetworkSlitherProvider)
        assert isinstance(self._file_provider, FileSlitherProvider)
        assert isinstance(self.net_info, NetworkInfoProvider)

        start_time = time.time()

        self._v1 = get_contract_data_from_address(
            self._v1_address, "", self._network_provider, self.net_info, suffix="V1"
        )
        self._v2 = get_contract_data_from_path(
            self._v2_path, self.out_dir, self._file_provider, suffix="V2"
        )
        self._v2["address"] = "0x0102030405060708091011121314151617181920"
        ...
        if self._proxy_address is not None:
            ...
            if is_address(self._proxy_address):
                ...
                CryticPrint.print_error(
                    "\n  * When using hybrid mode, the proxy must be specified as an address.",
                )
                self._proxy = None
        else:
            self._proxy = None
        ...
        if self._target_addresses is not None:
            ...
            self._targets, _, _ = get_contracts_from_comma_separated_string(
                self._target_addresses, self._network_provider, self.net_info
            )
        else:
            self._targets = None
